...
export const TaskNode: React.FC<TaskNodeProps> = ({ task, depth, isDragOver = false, hideSourceWhileDragging = false, compact = false }) => {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`flex items-start gap-2 rounded-md group relative pe-[4.75rem] ${
    ...
    >
      ...
      <div className={`absolute end-0 flex items-center opacity-0 group-hover:opacity-100 transition-opacity -translate-y-1/2 ${compact ? 'top-[17px]' : 'top-[24px]'}`}>
        {task.type === 'task' && (
